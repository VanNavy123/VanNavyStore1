...
function product_brands_filter_query( $filtered_posts ) {
	...
	if ( is_active_widget( false, false, 'mc_brands_filter', true ) && ! is_admin() ) {
		...
		if ( ! empty( $_GET[ 'filter_product_brand' ] ) && taxonomy_exists( $taxonomy ) ) {

    		$_chosen_brands[ $taxonomy ]['terms'] = explode( ',', $_GET[ 'filter_product_brand' ] );

    		if ( empty( $_GET[ 'query_type_product_brand' ] ) || ! in_array( strtolower( $_GET[ 'query_type_product_brand' ] ), array( 'and', 'or' ) ) )
    			$_chosen_brands[ $taxonomy ]['query_type'] = 'and';
    		else
    			$_chosen_brands[ $taxonomy ]['query_type'] = strtolower( $_GET[ 'query_type_product_brand' ] );
