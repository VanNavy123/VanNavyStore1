	...
	function woocommerce_products_live_search() {
		if ( isset( $_REQUEST['fn'] ) && 'get_ajax_search' == $_REQUEST['fn'] ) {
			...
			if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '2.7', '<' ) ) {
				$query_args = apply_filters( 'mc_live_search_query_args', array(
					'posts_per_page' 	=> 10,
					'no_found_rows' 	=> true,
					'post_type'			=> 'product',
					'post_status'		=> 'publish',
			...
			foreach ( $product_objects as $product_object ) {
				$id = mc_wc_get_product_id( $product_object );
				$title = get_the_title( $id );
				$title = html_entity_decode( $title , ENT_QUOTES, 'UTF-8' );
				$price = $product_object->get_price_html();
				$brand = woocommerce_show_brand( $id, true );
				...
				$results[] = apply_filters( 'mc_live_search_results_args', array(
					'value' 	=> $title,
